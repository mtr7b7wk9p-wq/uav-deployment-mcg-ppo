...
Each resource task represents an area-band pair, with 3 bands by default. Spectrum occupancy and demand intensity are hidden states with independent local estimates, uncertainty, age, and confidence. Each task now also has a hidden dynamic business queue and stochastic arrival rate. Sensing and accepted neighbor messages update local queue beliefs; the true queue is never exposed directly. Link quality is computed from local UAV-task geometry, and remaining UAV time is the local energy proxy. Resource local observations use 6 self features, 8 task slots with 17 features each, and 2 received-message slots with 17 features each, for a default dimension of 176. Resource actions are movement, sensing a visible task slot, or scheduling a visible task slot. Scheduling now computes actual served data from queue backlog, spectrum availability, link quality, same-band interference, and service energy, then reports service rate, demand satisfaction, priority assurance, queue overflow, and service energy consumption. Old resource checkpoints are incompatible with this observation layout.
